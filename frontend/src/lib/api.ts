...
export const api = {
  ...
  updateProfile: (full_name?: string, avatar_url?: string) =>
    request<{ success: boolean; user?: any; error?: string }>('/api/v2/auth/profile', {
      method: 'PUT', body: JSON.stringify({ full_name, avatar_url })
    }),

  changePassword: (current_password: string, new_password: string) =>
    request<{ success: boolean; message?: string; error?: string }>('/api/v2/auth/password', {
